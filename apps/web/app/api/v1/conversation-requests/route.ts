import { conversationRequestInputSchema, messageRequestBoxQuerySchema } from "@campus-exchange/contracts";
...
export async function GET(request: Request) {
  ...
  const parsed = messageRequestBoxQuerySchema.safeParse({
    box: url.searchParams.get("box") ?? "incoming",
    cursor: url.searchParams.get("cursor") ?? undefined,
    limit: url.searchParams.get("limit") ?? 20
  });
  if (!parsed.success) return apiError(request, 400, "bad_request", "Choose incoming or sent requests.");
  const { data, error } = await context.supabase.rpc("conversation_request_box_v2", {
    requested_box: parsed.data.box,
    after_cursor: parsed.data.cursor ?? null,
    result_limit: parsed.data.limit
  });
  return error ? apiError(request, 500, "internal_error", "Unable to load message requests.") : apiData(request, data ?? { items: [], counts: { incoming: 0, sent: 0, pending: 0 }, nextCursor: null });
}
