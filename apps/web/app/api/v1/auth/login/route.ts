...
export async function POST(request: Request) {
  ...
  const limited = await enforceRateLimits(request, [
    { scope: "password-login-ip", subject: clientAddress, limit: 50, windowSeconds: 600 },
    { scope: "password-login-account", subject: input.identifier.toLowerCase(), limit: 10, windowSeconds: 600 }
  ...
  try {
    ...
    const [profileResult, enforcementResult] = await Promise.all([supabase.from("profiles").select("status,onboarding_completed_at,password_setup_required").eq("id", data.user.id).maybeSingle(),supabase.rpc("auth_v2_enforced")]);
    const profile = profileResult.data;
    let authV2Enforced: boolean;
    try { authV2Enforced = requireBooleanSetting(enforcementResult, "auth_v2_enforcement"); }
    catch {
