...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (typeof body.grantId === "string") {
      const { data, error } = await db.rpc("complete_registration_enrollment", { enrollment_grant_id: body.grantId });
      if (error) {
        return apiError(request, 403, "forbidden", "This enrollment verification is invalid, expired, or already used.");
      }
      enrollment = (data?.[0] as Record<string, unknown> | undefined) ?? null;
    }
    ...
    if (!profile.onboarding_completed_at || profile.password_setup_required) return apiData(request, { next: "/onboarding", enrollment });
    ...
    return apiData(request, { next: "/home", enrollment });
  } catch {
