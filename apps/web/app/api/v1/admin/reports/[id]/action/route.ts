...
export async function POST(request:Request,{params}:Params){const e=verifyMutationOrigin(request);if(e)return e;const c=await requireStaffCapability(request,"cases.act");if(c instanceof NextResponse)return c;const input=await parseJson(request,schema);if(input instanceof NextResponse)return input;const {id}=await params;const {error}=await c.supabase.rpc("moderate_report",{target_report:id,chosen_action:input.action,action_reason:input.reason});return error?apiError(request,409,"conflict",error.message):apiData(request,{resolved:true});}
