...
export async function GET(request: Request,{params}:Params){
 const context=await requireStaffCapability(request,"cases.read");if(context instanceof NextResponse)return context;const{id}=await params;
 const {data:selected,error}=await context.supabase.from("moderation_cases").select("*").eq("id",id).single();if(error||!selected)return apiError(request,404,"not_found","Moderation case not found.");
 const [{data:report},{data:events},{data:actions},{data:appeals},{data:related}]=await Promise.all([
  ...
  context.supabase.from("moderation_cases").select("id,status,severity,created_at").eq("entity_type",selected.entity_type).eq("entity_id",selected.entity_id).neq("id",id).order("created_at",{ascending:false}).limit(20)
 ]);
 ...
 return apiData(request,{case:selected,report,events:events??[],actions:actions??[],appeals:appeals??[],profiles:profiles??[],related:related??[]});
}