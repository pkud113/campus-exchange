...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await context.supabase.rpc("reverse_admin_operational_action", {
    target_case: id,
    submitted_reason: input.reason,
    request_key: input.idempotencyKey
  });
  return error ? mutationError(request, error, "Unable to reverse the administrative action.") : apiData(request, { operationalCaseId: data, reversed: true });
}
