...
export async function GET(request: Request, { params }: Params) {
  ...
  const [{ data: members }, { data: membershipQueue }, { data: viewerMembership }, { data: events }, { data: posts }, { data: viewerCapabilities }, { data: notificationPreferences }] = await Promise.all([
    ...
    context.supabase.from("organization_notification_preferences").select("announcements,mentions,membership_changes,muted_until").eq("organization_id", organization.id).eq("profile_id", context.userId).maybeSingle(),
  ]);
  return apiData(request, { ...organization, members: members ?? [], membershipQueue: membershipQueue ?? [], viewerMembership: viewerMembership ?? null, viewerCapabilities: viewerCapabilities?.[0] ?? null, notificationPreferences: notificationPreferences ?? null, upcomingEvents: events ?? [], recentPosts: posts ?? [] });
}
