...
export async function GET(request: Request) {
  ...
  const url = new URL(request.url); const page = cursorSchema.parse(Object.fromEntries(url.searchParams)); const cursor = decodeCursor(page.cursor);
  let query = context.supabase.from("listings").select("id,campus_id,seller_id,title,description,category,condition,price_cents,currency,status,created_at,profiles!listings_seller_id_fkey(handle,display_name),media_uploads(id,alt_text)").eq("campus_id", context.campusId).eq("status", "active").order("created_at", { ascending: false }).order("id", { ascending: false }).limit(page.limit + 1);
  ...
  return NextResponse.json({ data: visible, page: { nextCursor: hasMore && last ? encodeCursor(last.created_at, last.id) : null } }, { headers: { "cache-control": "private, max-age=15", "x-request-id": context.requestId } });
}
