...
export async function GET(request: Request) {
  ...
  const { data, error } = await context.supabase.from("profiles").select("id,campus_id,handle,display_name,bio,academic_field,graduation_year,graduation_year_visible,interests,profile_visibility,status,verified_at,verified_until,created_at,avatar_media_id,banner_media_id,campuses(name,short_name,slug)").eq("id", context.userId).single();
...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await context.supabase.from("profiles").update({ display_name: input.displayName, bio: input.biography, academic_field: input.academicField, graduation_year: input.graduationYear, graduation_year_visible: input.graduationYearVisible, interests: input.interests, profile_visibility: input.visibility }).eq("id", context.userId).select("id,handle,display_name,bio,academic_field,graduation_year,graduation_year_visible,interests,profile_visibility,avatar_media_id,banner_media_id").single();
