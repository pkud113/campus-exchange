...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const { slug } = await params;
  const {data:targetCommunity,error:targetError}=await context.supabase.from("discussion_communities").select("id,display_name,description,rules").eq("slug",slug.toLowerCase()).single();
  if(targetError||!targetCommunity)return apiError(request,404,"not_found","Community not found.");
