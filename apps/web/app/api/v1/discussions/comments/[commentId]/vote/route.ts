...
export async function POST(request: Request, { params }: Params) {
  ...
  const { commentId } = await params; const { data, error } = await context.supabase.rpc("set_discussion_vote", { target_type: "comment", target_id: commentId, desired_value: input.value });
