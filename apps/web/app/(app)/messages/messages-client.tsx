...
export function MessagesClient() {
  ...
  const [memberQuery, setMemberQuery] = useState(params.get("new") ?? "");
  const [members, setMembers] = useState<Profile[]>([]);
  const [searching, setSearching] = useState(false);
  const loadInbox = useCallback(async () => {
    const [conversationResponse, requestResponse] = await Promise.all([
      fetch("/api/v1/conversations"),
      fetch("/api/v1/conversation-requests"),
    ]);
    const [conversationBody, requestBody] = await Promise.all([
  ...
  useEffect(() => {
    createSupabaseBrowserClient()
      .auth.getUser()
      .then(({ data }) => setCurrentUser(data.user?.id ?? ""));
    void loadInbox();
  }, [loadInbox]);
  useEffect(() => {
    if (params.get("new")) {
      void searchMembers(params.get("new") ?? "");
    }
  }, []);
  useEffect(() => {
    if (!selected) {
    ...
    async function load() {
      const response = await fetch(
        `/api/v1/conversations/${selected}/messages`,
      );
      const result = await response.json();
      if (response.ok && live) {
        setMessages([...(result.data ?? [])].reverse());
        await fetch(`/api/v1/conversations/${selected}/read`, {
          method: "POST",
  ...
  async function searchMembers(query = memberQuery) {
    if (query.trim().length < 2) return;
    setSearching(true);
    const response = await fetch(
      `/api/v1/profiles?q=${encodeURIComponent(query.trim())}`,
    );
    const result = await response.json();
    if (response.ok) setMembers(result.data);
    else setError(result.error?.message ?? "Unable to search members.");
    setSearching(false);
  }
  async function requestConversation(profileId: string) {
    setError("");
    ...
    const result = await response.json();
    if (response.ok) {
      setMembers((items) => items.filter((item) => item.id !== profileId));
      await loadInbox();
    } else setError(result.error?.message ?? "Unable to send request.");
  }
  async function respond(id: string, responseValue: "accepted" | "declined") {
    ...
    else setError(result.error?.message ?? "Unable to update request.");
  }
  ...
  async function blockActiveMember() {
    if (!active) return;
    setError("");
    const response = await fetch(
      `/api/v1/blocks/${active.other_profile_id}`,
    ...
    if (response.ok) {
      ...
      setError(result?.error?.message ?? "Unable to block this member.");
    }
  }
  if (loading)
    return (
  ...
  return (
    <main className="messages-page">
      <section className="conversation-list">
        <div className="messages-head">
          ...
          >
            <label>
              <Search />
              <input
                placeholder="Find campus members"
                aria-label="Find campus members"
                value={memberQuery}
            ...
            {members.map((member) => (
              <article key={member.id}>
                ...
                <button
                  ...
                  onClick={() => requestConversation(member.id)}
                >
                  <UserPlus />
                  Request
                </button>
        ...
        {searching && <p className="form-notice">Searching…</p>}
        {requests.length > 0 && (
          <div className="request-list">
            <h2>Conversation requests</h2>
            {requests.map((request) => {
              const incoming = request.recipient_id === currentUser;
              const person = one(
                incoming ? request.requester : request.recipient,
              ) ?? {
                id: "",
                handle: "member",
              ...
              return (
                <article key={request.id}>
                  ...
                  <div>
                    ...
                    <span>
                      {incoming ? "wants to message you" : "Request pending"}
                    </span>
                  </div>
                  {incoming && (
                    <>
        ...
        {!conversations.length && (
          <div className="empty-state">
            ...
            <p>Search for a campus member or open a listing to begin.</p>
          </div>
        )}
        {conversations.map((conversation) => (
          ...
          >
            ...
            <div>
              <span>
                <strong>
                  {conversation.other_display_name ?? conversation.other_handle}
                </strong>
                <small>
                  {new Date(
            ...
            {Number(conversation.unread_count) > 0 && (
              <b className="nav-badge">{conversation.unread_count}</b>
            )}
          </button>
        ))}
      </section>
      <section className="chat-panel">
          ...
          <>
            <header>
              ...
              <div>
                ...
                <small>
                  {active.listing_title ?? `@${active.other_handle}`}
                </small>
              </div>
              <button type="button" className="thread-block" onClick={blockActiveMember}>
                <Ban /> Block
              </button>
            </header>
            <div className="trade-banner">
              <MessageCircle />
              <div>
                <strong>Keep the conversation here</strong>
                <span>
                  Never share verification codes or send a deposit to hold an
                  item.
                </span>
              </div>
            </div>
            <div className="message-stream">
              {messages.length ? (
                messages.map((message) => (
                  <div
                    key={message.id}
                    className={`bubble ${message.sender_id === currentUser ? "mine" : "theirs"}`}
                  >
                    {message.body}
                    <small>
            ...
            <form className="message-composer" onSubmit={send}>
              <input
                ...
                maxLength={4000}
              />
              <button className="button button-primary" disabled={!body.trim()}>
                Send
              </button>
            </form>
          </>
        ...
        {error && (
          <p className="form-error message-error" role="alert">
            {error}
          </p>
        )}
      </section>
