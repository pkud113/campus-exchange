...
type Tab = "posts" | "listings" | "events" | "organizations" | "about";
...
export function ProfileExperience({ profile, own }: { profile: Profile; own: boolean }) {
  const [tab, setTab] = useState<Tab>("posts"); const [items, setItems] = useState<Item[]>([]); const [nextCursor, setNextCursor] = useState<string | null>(null); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  ...
  useEffect(() => { setItems([]); setNextCursor(null); void load(tab); }, [load, tab]);

  return <section className="profile-content">
    <nav className="profile-tabs" aria-label="Profile sections">{tabs.map((item) => <button key={item.id} className={tab === item.id ? "active" : ""} aria-current={tab === item.id ? "page" : undefined} onClick={() => setTab(item.id)}>{item.icon}{item.label}</button>)}</nav>
    <header className="profile-tab-header"><div><h2>{tabs.find((item) => item.id === tab)?.label}</h2><p>{tab === "posts" ? "Visual updates and text posts" : tab === "listings" ? "Marketplace activity" : tab === "events" ? "Hosted organization and student events" : tab === "organizations" ? "Visible campus memberships and leadership" : "Student identity and campus context"}</p></div>{own && tab === "posts" && <Link className="button button-primary button-small" href="/social#composer"><Plus /> Create post</Link>}{own && tab === "listings" && <Link className="button button-primary button-small" href="/sell"><Plus /> Create listing</Link>}{own && tab === "events" && <Link className="button button-primary button-small" href="/events/new"><Plus /> Create event</Link>}</header>
    ...
    {!loading && !items.length && tab !== "about" && tab !== "organizations" && <EmptyState icon={tab === "posts" ? <Grid3X3 /> : tab === "listings" ? <ShoppingBag /> : <CalendarDays />} title={`No ${tab} to show`} description={own ? `Your ${tab} will appear here when you add them.` : `This student has no visible ${tab}.`} />}
    {tab === "posts" && <div className="profile-post-gallery">{items.map((post) => { const media = post.social_post_media?.sort((a: any,b: any) => a.position-b.position)?.[0]; return <Link href={`/social/posts/${post.id}`} key={post.id} className={media ? "profile-post-tile image" : "profile-post-tile text"}>{media ? <img src={`/api/v1/media/${media.media_id}?variant=medium`} alt={post.body.slice(0, 100)} /> : <blockquote>{post.body}</blockquote>}<span>{post.reaction_count} reactions · {post.comment_count} comments</span></Link>; })}</div>}
