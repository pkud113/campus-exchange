...
export async function AppShell({children}:{children:React.ReactNode}){
  ...
  return <div className="app-frame"><AppNavigation profile={{id:user.id,handle:profile?.handle??"member",displayName:profile?.display_name??profile?.handle??"Campus member",avatarId:profile?.avatar_media_id??null,campusName:campus?.name??"Your campus",verified:isStaff||Boolean(profile?.verified_until&&new Date(profile.verified_until)>new Date())}} isStaff={isStaff} notificationCount={notificationCount??0} messageCount={messageCount}/><div className="app-main">{children}</div></div>;
}
