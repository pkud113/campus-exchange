...
describe("notificationHref", () => {
  it("maps legacy routes to current App Router destinations", () => {
    expect(notificationHref("/messages/requests", "message_request")).toBe("/messages?view=incoming");
    expect(notificationHref("/messages?view=sent", "message_request")).toBe("/messages?view=sent");
